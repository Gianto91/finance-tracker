...
# Middleware para desactivar caché en HTML e imágenes
@app.middleware("http")
async def no_cache_middleware(request: Request, call_next):
    response = await call_next(request)
    # No-cache para HTML y imágenes
    if request.method == "GET" and (request.url.path == "/" or request.url.path.startswith("/images/")):
